...
eth_proffit_array = [[3, 2], [4, 2], [5, 2], [7, 2], [10, 2]] #, [17, 0], [20, 0]]#, [200, 1], [200, 0]]
    ...
    try:
        ...
        if open_sl == "":  # no position
            ...
            if signal == 'long':
                ...
                prt('открыта позиция ' + symbol + ' Long ' + str(maxposition) + ' ' + str(round(j*maxposition, 3)))

            elif signal == 'short':
                open_position(symbol, 'short', maxposition)
                j = get_symbol_price(symbol)
                prt('открыта позиция ' + symbol + ' Short ' + str(maxposition) + ' ' + str(round(j*maxposition, 3)))
            ...
            if open_sl == 'long':
                ...
                if (current_price < stop_price) or signal == 'short1':
                    ...
                    prt('закрыта позиция по СТОПУ ' + symbol + ' long ' + str(quantity) + ' ' + str(round(sumlong, 3))+ ' profit ' + str(round(sum(sum_profit),3)))
                    proffit_array = copy.copy(eth_proffit_array)
                else:
                    ...
                    for j in range(0, len(temp_arr) - 1):
                        ...
                        if (current_price > (entry_price + delta)):
                            ...
                            sum_profit.append(sumlong)
                            prt('закрыта позиция ' + symbol + ' long количество: ' + str(round(maxposition * (contracts / 10), 3)) + ' сумма ' + str(round(sumlong,3))+ ' profit ' + str(round(sum(sum_profit),3)))
                            del proffit_array[0]
            ...
            if open_sl == 'short':
                ...
                if (current_price > stop_price) or signal == 'long1':
                    ...
                    sum_profit.append(sumlong)
                    prt('закрыта позиция по СТОПУ ' + symbol + ' short ' + str(quantity) + ' ' + str(round(sumlong,3))+ ' profit ' + str(round(sum(sum_profit),3)))
                    proffit_array = copy.copy(eth_proffit_array)
                else:
                    temp_arr = copy.copy(proffit_array)
                    for j in range(0, len(temp_arr) - 1):
                        ...
                        if (current_price < (entry_price - delta)):
                            ...
                            sum_profit.append(sumlong)
                            prt('закрыта позиция ' + symbol + ' short количество: ' + str(round(maxposition * (contracts / 10), 3)) + ' сумма ' + str(round(sumlong,3)) + ' profit ' + str(round(sum(sum_profit),3)))
